    ...
    try {
        for (const col of collectionNames) {
            ...
            querySnapshot.forEach((doc) => {
                ...
                const nameContains = doc.data().name.toUpperCase().includes(searchTermUpper)

                // Check if the description contains the search term
                const descSplit = doc.data().description.split(' ').map((part: string) => part.toUpperCase());
                const descContains = descSplit.some((part: string | string[]) => part.includes(searchTermUpper));
